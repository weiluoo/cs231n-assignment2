...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            if self.normalization == 'batchnorm':
                out, cache = affine_bn_relu_forward(out, self.params['W%d' % i], self.params['b%d' % i],
                                                    self.params['gamma%d' % i], self.params['beta%d' % i],
                                                    self.bn_params[i - 1])
            elif self.normalization == 'layernorm':
                out, cache = affine_ln_relu_forward(out, self.params['W%d' % i], self.params['b%d' % i],
                                                    self.params['gamma%d' % i], self.params['beta%d' % i],
                                                    self.ln_params[i - 1])
            else:
            ...
            L2reg += 0.5 * self.reg * np.sum(self.params['W%d' % i] ** 2)
        i += 1
        scores, cache = affine_forward(out, self.params['W%d' % i], self.params['b%d' % i])
        cache_history.append(cache)
        L2reg += 0.5 * self.reg * np.sum(self.params['W%d' % i] ** 2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        loss += L2reg
        dout, grads['W%d' % i], grads['b%d' % i] = affine_backward(dout, cache_history.pop())
        grads['W%d' % i] += self.reg * self.params['W%d' % i]
        i -= 1
        while i > 0:
            if self.use_dropout:
                cache, do_cache = cache_history.pop()
                dout = dropout_backward(dout, do_cache)
            else:
                cache = cache_history.pop()
            if self.normalization == 'batchnorm':
                grads['gamma%d' % i], grads['beta%d' % i], dout, grads['W%d' % i], grads['b%d' % i] = \
